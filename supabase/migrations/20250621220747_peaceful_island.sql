...
BEGIN
  ...
  DELETE FROM auth.users WHERE email LIKE '%@BitCapro.com' AND id::text LIKE '00000000-%';
...
INSERT INTO user_profiles (email, name, role, department, active) VALUES
  ('admin@BitCapro.com', 'System Administrator', 'Admin', 'IT', true),
  ('ceo@BitCapro.com', 'Emily Davis', 'Approver_L4', 'Executive', true),
  ('cfo@BitCapro.com', 'Robert Chen', 'Approver_L3', 'Finance', true),
  ('director1@BitCapro.com', 'Sarah Wilson', 'Approver_L2', 'Operations', true),
  ('manager1@BitCapro.com', 'Mike Johnson', 'Approver_L1', 'Operations', true),
  ('john.doe@BitCapro.com', 'John Doe', 'Submitter', 'Engineering', true)
ON CONFLICT (email) DO UPDATE SET
  name = EXCLUDED.name,
  role = EXCLUDED.role,
