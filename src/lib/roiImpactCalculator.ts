...
export class ROIImpactCalculator {
  ...
  static calculateROIImpact(
    ...
    const delayInWeeks = Math.max(0, (finalDate.getTime() - submission.getTime()) / (1000 * 60 * 60 * 24 * 7));
    ...
    const roiLossPercentage = originalROI > 0 ? (roiLoss / originalROI) * 100 : 0;
  ...
  static generateROITimeline(
    ...
    const submission = new Date(submissionDate);
    const endDate = finalApprovalDate ? new Date(finalApprovalDate) : new Date();
    
    const totalWeeks = Math.ceil((endDate.getTime() - submission.getTime()) / (1000 * 60 * 60 * 24 * 7));
    
    for (let week = 0; week <= totalWeeks; week++) {
      const currentDate = new Date(submission.getTime() + (week * 7 * 24 * 60 * 60 * 1000));
  ...
  static calculateSummaryStats(impacts: ROIImpactData[]): ROIImpactSummary {
    ...
    const totalDelay = impacts.reduce((sum, impact) => sum + impact.delayInWeeks, 0);
    const totalROILoss = impacts.reduce((sum, impact) => sum + impact.roiLoss, 0);
    const totalValueLost = impacts.reduce((sum, impact) => sum + impact.lostValue, 0);
    
    const delays = impacts.map(impact => impact.delayInWeeks).filter(delay => delay > 0);
...
export const calculateDynamicDecayRate = ROIImpactCalculator.calculateDynamicDecayRate;
