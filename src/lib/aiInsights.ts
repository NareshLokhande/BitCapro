import { ApprovalLog, InvestmentRequest, KPI } from './supabase';
...
export class AIInsightsEngine {
  ...
  static analyzeInvestmentRequest(
    ...
    // Financial Analysis
    insights.push(
      ...AIInsightsEngine.analyzeFinancials(request, allRequests, kpis),
    );

    // Business Case Analysis
    insights.push(...AIInsightsEngine.analyzeBusinessCase(request));

    // Compliance Analysis
    insights.push(...AIInsightsEngine.analyzeCompliance(request));

    // Process Optimization
    insights.push(...AIInsightsEngine.analyzeProcess(request, allRequests));

    // Strategic Analysis
    insights.push(...AIInsightsEngine.analyzeStrategic(request, allRequests));
  ...
  static predictApprovalDelay(
    ...
    const similarRequests = AIInsightsEngine.findSimilarRequests(
      request,
      allRequests,
    );
    const historicalDelays = AIInsightsEngine.calculateHistoricalDelays(
      similarRequests,
      approvalLogs,
    );
    ...
    if (totalAmount > 500000) {
      ...
      riskFactors.push(
        'High investment amount (>$500K) typically requires additional review',
      );
      recommendations.push(
        'Prepare detailed financial justification and ROI analysis',
      );
    }
    ...
    if (request.business_case_type && request.business_case_type.length > 2) {
      ...
      riskFactors.push(
        'Multiple business case types increase review complexity',
      );
      recommendations.push(
        'Clearly separate and justify each business case type',
      );
    }
    ...
    if (request.business_case_type?.includes('ESG')) {
      if (
        !request.carbon_footprint_data ||
        Object.keys(request.carbon_footprint_data).length === 0
      ) {
        predictedDelayDays += 7;
        riskFactors.push(
          'ESG cases without carbon footprint data face significant delays',
        );
        recommendations.push(
          'Complete carbon footprint assessment before submission',
        );
      }
    }
    ...
    if (request.business_case_type?.includes('IPO Prep')) {
      if (!request.stock_exchange_target) {
        ...
        riskFactors.push(
          'IPO Prep cases require target stock exchange specification',
        );
        recommendations.push(
          'Specify target stock exchange and provide market analysis',
        );
      }
    }
    ...
    if (historicalDelays.averageDelay > 0) {
      predictedDelayDays = Math.round(
        (predictedDelayDays + historicalDelays.averageDelay) / 2,
      );
    ...
    if ([11, 0].includes(currentMonth)) {
      ...
      predictedDelayDays += 3;
      riskFactors.push(
        'Year-end/New year period typically sees processing delays',
      );
    }
  ...
  static optimizeBusinessCase(
    ...
    if (!currentTypes.includes('ESG')) {
      if (
        ...
        potentialBenefits.push(
          'Improved ESG ratings and stakeholder perception',
        );
        reasoning +=
          'Project shows environmental benefits that qualify for ESG classification. ';
        confidenceScore += 15;
      }
    }
    ...
    if (!currentTypes.includes('Cost Control')) {
      if (
        ...
        description.includes('reduce cost') ||
    ...
    if (!currentTypes.includes('Expansion')) {
      if (
        description.includes('capacity') ||
        description.includes('growth') ||
        description.includes('market') ||
        description.includes('expansion') ||
        totalAmount > 200000
      ) {
        suggestedTypes.push('Expansion');
        potentialBenefits.push(
          'Strategic priority status for growth initiatives',
        );
    ...
    if (!currentTypes.includes('Compliance')) {
      if (
        description.includes('regulatory') ||
        ...
        reasoning +=
          'Project addresses regulatory or compliance requirements. ';
        confidenceScore += 8;
      }
    }
    ...
    return {
      ...
      reasoning:
        reasoning ||
        'Current business case types appear optimal for this project.',
  ...
  private static analyzeFinancials(
    ...
    if (totalAmount > 1000000) {
      insights.push({
        ...
        description: `Investment of $${(totalAmount / 1000000).toFixed(
          1,
        )}M requires enhanced scrutiny`,
        recommendation:
          'Consider phasing the investment over multiple years to reduce risk and improve cash flow',
        impact:
          'May face additional approval delays and require board-level review',
        confidence: 90,
        category: 'financial',
        actionable: true,
    ...
    if (capexRatio > 0.8 && totalAmount > 200000) {
      insights.push({
        ...
        recommendation:
          'Consider leasing or service models to convert some CapEx to OpEx for better cash flow',
    ...
    if (requestKPIs.length === 0 && totalAmount > 50000) {
      insights.push({
        ...
        recommendation:
          'Add comprehensive financial analysis including IRR, NPV, and payback period',
        impact:
          'Likely to cause approval delays and reduce approval probability',
        confidence: 95,
        category: 'financial',
        actionable: true,
    ...
    if (requestKPIs.length > 0) {
      const avgIRR =
        requestKPIs.reduce((sum, kpi) => sum + kpi.irr, 0) / requestKPIs.length;
      if (avgIRR < 10) {
        insights.push({
          ...
          description: `Average IRR of ${avgIRR.toFixed(
            1,
          )}% is below typical investment thresholds`,
          recommendation:
            'Review project scope to identify additional value drivers or cost reductions',
  ...
  private static analyzeBusinessCase(request: InvestmentRequest): AIInsight[] {
    ...
    if (businessCaseTypes.length === 0) {
      insights.push({
        ...
        recommendation:
          'Select at least one appropriate business case type to enable proper routing',
    ...
    if (businessCaseTypes.length > 3) {
      insights.push({
        ...
        recommendation:
          'Focus on 1-2 primary business case types for clearer approval path',
  ...
  private static analyzeCompliance(request: InvestmentRequest): AIInsight[] {
    ...
    if (businessCaseTypes.includes('ESG')) {
      if (
        ...
        insights.push({
          ...
          recommendation:
            'Complete carbon footprint analysis including baseline, methodology, and reduction targets',
      ...
      if (
        ...
        insights.push({
          ...
          recommendation:
            'Upload ESG reports, sustainability plans, or environmental impact assessments',
    ...
    if (businessCaseTypes.includes('IPO Prep')) {
      if (!request.stock_exchange_target) {
        insights.push({
          ...
          description:
            'IPO Prep business case requires target stock exchange specification',
          recommendation:
            'Specify target stock exchange (NYSE, NASDAQ, etc.) and provide market analysis',
          impact:
            'IPO preparation approval cannot proceed without exchange target',
          confidence: 100,
          category: 'compliance',
          actionable: true,
    ...
    if (businessCaseTypes.includes('Compliance')) {
      if (
        ...
        Object.keys(request.regulatory_requirements).length === 0
      ) {
        insights.push({
          ...
          description:
            'Compliance business case requires regulatory requirements documentation',
          recommendation:
            'Document specific regulatory requirements, deadlines, and compliance frameworks',
    ...
    const incompleteItems = complianceItems.filter(
      (item) => !request[item.field as keyof InvestmentRequest],
    );
    ...
    if (incompleteItems.length > 2) {
      insights.push({
        ...
        recommendation: `Complete the following items: ${incompleteItems
          .map((i) => i.name)
          .join(', ')}`,
        impact:
          'May delay approval process and require additional review cycles',
        confidence: 80,
        category: 'compliance',
        actionable: true,
  ...
  private static analyzeProcess(
    ...
    if (projectDuration > 3 && totalAmount > 500000) {
      insights.push({
        ...
        recommendation:
          'Consider breaking into phases with milestone-based funding releases',
    ...
    const deptRequests = allRequests.filter(
      (r) =>
    ...
    if (deptRequests.length > 5) {
      insights.push({
        ...
        recommendation:
          'Consider prioritizing this request or adjusting timeline expectations',
        impact:
          'May experience longer approval times due to department workload',
        confidence: 70,
        category: 'process',
        actionable: true,
        estimatedTimeToImplement: '1 day',
  ...
  private static analyzeStrategic(
    request: InvestmentRequest,
    allRequests: InvestmentRequest[],
  ): AIInsight[] {
    const insights: AIInsight[] = [];
    ...
    if (!request.strategic_fit) {
      insights.push({
        ...
        recommendation:
          'Review and confirm alignment with company strategic objectives',
        impact:
          'May face questions during approval process about strategic value',
        confidence: 80,
        category: 'strategic',
        actionable: true,
        estimatedTimeToImplement: '2-3 days',
    ...
    // Portfolio analysis
    const deptInvestments = allRequests.filter(
      (r) => r.department === request.department && r.status === 'Approved',
    );
    const deptTotalInvestment = deptInvestments.reduce(
      (sum, r) => sum + r.base_currency_capex + r.base_currency_opex,
      0,
    );
    ...
    if (totalAmount > deptTotalInvestment * 0.5) {
      insights.push({
        ...
        description: `This investment represents ${(
          (totalAmount / deptTotalInvestment) *
          100
        ).toFixed(0)}% of department's approved investments`,
        recommendation:
          'Ensure this investment aligns with department strategic priorities and resource allocation',
        impact:
          'High visibility investment that may require additional strategic justification',
        confidence: 65,
        category: 'strategic',
        actionable: true,
  ...
  private static findSimilarRequests(
    request: InvestmentRequest,
    allRequests: InvestmentRequest[],
  ): InvestmentRequest[] {
    const totalAmount =
    ...
    return allRequests.filter((r) => {
      ...
      const amountSimilar =
        Math.abs(totalAmount - rTotalAmount) / totalAmount < 0.5;
      const deptSimilar = r.department === request.department;
      const categorySimilar = r.category === request.category;
      const businessCaseSimilar =
        request.business_case_type?.some((type) =>
          r.business_case_type?.includes(type),
        ) || false;

      return (
        (amountSimilar && deptSimilar) ||
        (categorySimilar && businessCaseSimilar)
      );
    });
  }
